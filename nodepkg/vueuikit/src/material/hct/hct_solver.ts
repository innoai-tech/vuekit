...
export class HctSolver {
  ...
  private static sanitizeRadians(angle: number): any {
    return (angle + Math.PI * 8) % (Math.PI * 2);
  }
  private static trueDelinearized(rgbComponent: number): any {
  ...
  private static chromaticAdaptation(component: number): any {
    const af = Math.pow(Math.abs(component), 0.42);
    return (mathUtils.signum(component) * 400.0 * af) / (af + 27.13);
  }
  private static hueOf(linrgb: number[]): any {
  ...
  private static areInCyclicOrder(a: number, b: number, c: number): any {
    const deltaAB = HctSolver.sanitizeRadians(b - a);
    const deltaAC = HctSolver.sanitizeRadians(c - a);
    return deltaAB < deltaAC;
  }
  private static intercept(source: number, mid: number, target: number): any {
    return (mid - source) / (target - source);
  }
  private static lerpPoint(source: number[], t: number, target: number[]): any {
    return [
  ...
  private static isBounded(x: number): any {
    return 0.0 <= x && x <= 100.0;
  }
  private static nthVertex(y: number, n: number): any {
  ...
  private static bisectToSegment(y: number, targetHue: number): any {
  ...
  private static midpoint(a: number[], b: number[]): any {
    return [(a[0]! + b[0]!) / 2, (a[1]! + b[1]!) / 2, (a[2]! + b[2]!) / 2];
  }
  private static criticalPlaneBelow(x: number): any {
    return Math.floor(x - 0.5);
  }
  private static criticalPlaneAbove(x: number): any {
    return Math.ceil(x - 0.5);
  }
  private static bisectToLimit(y: number, targetHue: number): any {
  ...
  private static inverseChromaticAdaptation(adapted: number): any {
    const adaptedAbs = Math.abs(adapted);
    const base = Math.max(0, (27.13 * adaptedAbs) / (400.0 - adaptedAbs));
    return mathUtils.signum(adapted) * Math.pow(base, 1.0 / 0.42);
  }
  private static findResultByJ(hueRadians: number, chroma: number, y: number): any {
