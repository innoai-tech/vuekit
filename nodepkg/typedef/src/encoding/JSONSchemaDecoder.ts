...
export class JSONSchemaDecoder {
  ...
  decode(schema: JSONSchema): TypeAny {
    ...
    if (schema["oneOf"]) {
            ...
            if (s) {
              if (s.type == "object") {
                ...
                if (discriminatorType && discriminatorType.schema) {
                  for (const k in discriminatorType.schema) {
                    mapping[discriminatorType.schema[k]] = t.object(
      ...
      return t.union(...map(schema["oneOf"], (s) => this.decode(s)));
    }

    if (schema["allOf"]) {
      return t.intersection(...map(schema["allOf"], (s) => this.decode(s)));
    }
    ...
    if (isStringType(schema)) {
      // FIXME support schema.format === "binary"
      // if (schema.format === "binary") {
      //   return t.union(t.instanceOf(File), t.instanceOf(Blob));
      // }
      return t.string();
    }
