...
describe("define type", () => {
  describe("should create string", () => {
    ...
    describe("with optional", () => {
      ...
      expect(Schema.schemaAll(s2)).toEqual({
        type: "string",
      });
    ...
    describe("with modifiers", () => {
      ...
      expect(Schema.schemaAll(s2.schema)).toEqual({
        type: "string",
        minLength: 1,
        maxLength: 10,
      });
  ...
  for (const s of types) {
    test(s.type, () => {
      expect(Schema.schemaAll(s)).toMatchSnapshot();
    });
  }
});
...
describe("define type with metadata", () => {
  test("create object", () => {
    ...
    const s = Schema.schemaProp(t.object(Account), Schema.unwrap)();

    expect(Schema.schemaAll(s.schema)).toEqual({
      type: "object",
      properties: {
        name: {
          type: "string",
    ...
    for (const [k, _, prop] of s.entries({})) {
      ...
      metadata[k as string] = Schema.schemaAll(prop.meta);
    }
