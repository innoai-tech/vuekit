...
const B = /*#__PURE__*/String("b");`,
  ...
  },
  {
    ...
    dest: "export const A = fn( /*#__PURE__*/fn());",
  },
  {
...
describe("test cases", () => {
  for (const caseItem of cases) {
    ((caseItem as any).only ? it.only : it)(caseItem.title, async () => {
      const transformedCode = (await transform(caseItem.src)).code;

      const expectCode = (await format("test.ts", caseItem.dest, {})).code;
      const actualCode = (await format("test.ts", transformedCode, {})).code;
